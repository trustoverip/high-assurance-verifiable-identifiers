...
Sections 6.1–6.4 are non-normative. Section 6.5 is normative.
...
6.1. Domain Name System (DNS): Naming and Discoverability
...
6.3. Decentralized Identifiers (DIDs): Cryptographic Key Control
...
8.1. Requirements for Identifier Controllers
...
c) MUST store references in integrity-protected formats where available. For DNS records, this means DNSSEC (see Section 10.4 for deployment guidance). For X.509 certificates, this means CA-signed extensions. For DID Documents, this means the integrity mechanisms provided by the DID method.
...
For handling partial or failed validation states, see Section 14.
...
9.1. When Key Alignment is Appropriate
...
c) The Identifier Controller has assessed and accepted the risks described in Section 7 and the threat model in Section 16.
...
d) MAY request proof of possession (see Section 9.4).
...
9.5. Failure Scenario: Key Alignment Without Lifecycle Coordination
...
This scenario illustrates why key alignment MUST be accompanied by coordinated rotation (Section 9.2, item b) and why the effective cryptoperiod MUST track the shortest-lived system (Section 9.2, item c). When these requirements cannot be met, cross-endorsement without key alignment is the safer choice.

10. Bridging DIDs and DNS
...
See Section 10.1.4 for the trust implications of this inherent dependency.

10.1.2. Non-Web-Based DIDs
...
10.1.4. did:web and Circular Trust
...
The DNS zone containing the URI record MUST be integrity-protected (see Section 10.4).

DNS Query Target	Record Type	Returns
_did.<domain>	URI	The associated DID
10.3. Key Alignment: DID and DNS (Optional)
...
All normative requirements of Section 9 apply.

10.4. DNSSEC Deployment Considerations
...
Verifiers encountering DNS-based cross-endorsement records without DNSSEC validation MUST treat those records as integrity-unverified. This corresponds to State 5 (Integrity Failure) in Section 14. Verifiers MAY accept such records with reduced assurance if their risk model explicitly permits it, but MUST NOT treat them as equivalent to DNSSEC-validated records.

The HAVID ecosystem SHOULD track DNSSEC adoption trends and revisit this guidance as deployment matures.

11. Bridging DNS and X.509
...
11.2. Cross-Endorsement: DNS to X.509
...
b) Protecting the DNS zone with DNSSEC (see Section 10.4).
...
12.1. Cross-Endorsement: DID to X.509
...
12.2. Cross-Endorsement: X.509 to DID
...
Certificate Authorities issuing certificates with DID URIs in the SAN MUST follow the requirements in Section 20.1.

12.3. Key Alignment: DID and X.509 (Optional)
...
This pairing carries the highest lifecycle mismatch risk of the three pairings defined in this specification. X.509 certificates, particularly DV certificates, increasingly have short cryptoperiods (90 days or less). DIDs are often designed for longer-lived key relationships. See Section 9.5 for a concrete failure scenario.

When key alignment is implemented for this pairing:

a) All normative requirements of Section 9 apply.
...
13. Resolution and Dereferencing
...
13.2. Resolution Flows
13.2.1. Starting from a DID
...
Step 4. Optionally, verify key alignment per Section 9.
13.2.2. Starting from a DNS Domain
...
Step 5. Optionally, verify key alignment per Section 9.
13.2.3. Starting from an X.509 Certificate
...
Step 5. Optionally, verify key alignment per Section 9.
13.3. Resolution Behavior

Implementations SHOULD define timeout and retry policies for each resolution step. When one leg of a multi-identifier HAVID fails while others succeed, the Verifier MUST treat the failed leg as unresolved and assess the remaining identifiers according to the validation states in Section 14. Implementations MUST NOT block indefinitely on any single resolution step.
...
13.4. Establishing High Assurance
...
14. Validation States and Verifier Guidance

This section is normative.

14.1. Validation State Definitions

State 1: Full HAVID. Bi-directional cross-endorsement confirmed. Key alignment verified (where used). All integrity checks pass. All identifiers currently valid. Verifier action: Accept. Highest assurance.

State 2: Cross-Endorsed HAVID. Bi-directional cross-endorsement confirmed. Key alignment not used. All integrity checks pass. All identifiers currently valid. Verifier action: Accept. Standard assurance.

State 3: Partial Endorsement. Only unidirectional reference found (one identifier references the other, but the reverse is missing or unresolvable). Integrity passes on the available side. All identifiers currently valid. Verifier action: MUST NOT treat as a full HAVID. MAY accept with reduced assurance if the Verifier's risk model permits. SHOULD log for investigation.

State 4: Key Alignment Failure. Bi-directional cross-endorsement may be present, but key material does not match across identifiers. Verifier action: MUST NOT accept any key alignment claim. The cross-endorsement MAY still be valid independently as a State 2 (Cross-Endorsed HAVID) if it satisfies the requirements of Section 8 without relying on key alignment. State 1 is unreachable whenever key alignment has been attempted and failed, since the definition of State 1 above requires key alignment to be verified where used. SHOULD alert for potential lifecycle desynchronization.

State 5: Integrity Failure. One or more integrity checks fail (DNSSEC validation failure, certificate chain validation failure, DID method integrity failure). Verifier action: MUST NOT accept as HAVID. MUST treat the unverifiable reference as untrusted.

State 6: Expired or Revoked. One or more participating identifiers are expired, revoked, or deactivated. Verifier action: MUST NOT accept as HAVID. The Identifier Controller must re-establish the cross-endorsement with valid identifiers.

State 7: No Endorsement. No cross-endorsement references found between the identifiers. Verifier action: No HAVID exists.

14.2. Handling Degraded States
...
15. Identifying the Real-World Organization Behind an Identifier

This section is normative.

15.1. How Each Identifier Contributes
...
15.2. Resolution to a Legal Person

To identify the organization controlling a VID, a Verifier SHOULD:

a) Start from a known identifier.
b) Resolve linked identifiers via cross-endorsement (per Section 13).
...
16.2. Attack Goals and Mitigations

Fabricated cross-endorsement. An attacker creates a unidirectional reference from an identifier they control to an identifier they do not, making it appear that the two are linked. Mitigation: Bi-directional cross-endorsement requirement (Section 8) ensures both sides must assert the link. Verifiers MUST confirm both directions.

Stale key exploitation. An attacker obtains a compromised key that remains valid in one system (e.g., a DID Document) after it has been rotated in another (e.g., an X.509 certificate). Mitigation: Coordinated key rotation requirements (Section 9.2) and shortest-cryptoperiod rule.

Trust escalation via transitive inference. An attacker chains cross-endorsements to imply a relationship that was never directly asserted. Mitigation: The no-transitivity rule (Section 6.5) and maximum reference depth limits (Section 19.6).

DNS record manipulation. An attacker modifies _did URI or TLSA records to point to attacker-controlled identifiers. Mitigation: DNSSEC requirement for DNS-based cross-endorsements (Section 19.4). See also Section 10.4 for deployment pragmatics.
...
Note on did:webvh syntax: The did:webvh method requires a Self-Certifying Identifier (SCID) as the first element of the method-specific identifier. The format is did:webvh:<SCID>:<domain>:<optional-path>. The SCID is a globally unique value derived from the DID's initial log entry. In the examples below, z6Mkf5rGMoatrSj1f is used as a placeholder SCID. See the did:webvh specification for full details.
...
This example illustrates a realistic scenario in which several things go wrong at once, requiring a Verifier to apply multiple validation states from Section 14.

17.4.1. Scenario

MegaBank recently completed an infrastructure migration. During the migration, the following occurred:

DNS zone re-signing lapsed. The migration to a new DNS provider introduced a gap in DNSSEC signing. The _did.megabank.example URI record exists and points to did:webvh:z6MkpT9StBU:megabank.example:id, but the zone is currently unsigned.
X.509 certificate expired. The EV certificate for megabank.example expired three days ago. The renewal is in progress but has not yet completed. The expired certificate contains URI:did:webvh:z6MkpT9StBU:megabank.example:id in its SAN.
DID Document was updated with a new key. As part of the migration, MegaBank rotated its DID key material. The DID Document now contains a new verificationMethod with a new public key. The x5u reference still points to the expired certificate. The dnsValidationDomain is still set to megabank.example.
...
Per Section 14.2, the Verifier MUST NOT promote any of these to full HAVID assurance. The Verifier SHOULD surface all three failures to the relying application, log the degraded states, and MAY re-attempt after an interval to allow MegaBank to complete its migration.
...
Complete DNSSEC re-signing of the megabank.example zone.
Install the renewed EV certificate, ensuring the SAN still contains the DID URI.
Update the DID Document's x5u to reference the new certificate.
If key alignment is desired, ensure the new certificate's public key matches the DID Document's current verificationMethod.
...
The normative requirements of Section 9 are designed to mitigate the risks described in Section 7 and illustrated in Section 9.5. Identifier Controllers SHOULD prefer cross-endorsement alone when key alignment would introduce unacceptable lifecycle or governance complexity.
...
All DNS-based cross-endorsements (_did URI records, TLSA records, _lei NAPTR records) MUST be protected by DNSSEC. Verifiers MUST validate the full DNSSEC chain of trust. Unsigned or unverifiable DNS records MUST be treated as untrusted. See Section 10.4 for deployment pragmatics.
...
a) Verifiers MUST enforce a maximum reference depth as defined in Section 13.3. Implementations MUST NOT follow cross-endorsement chains beyond this limit.

b) Each reference in a chain MUST be validated independently; transitive trust MUST NOT be assumed by default (see also Section 6.5).
...
Expiration. Certificate expiration does not automatically invalidate the DID. These are separate but interlinked lifecycles. However, a Verifier MUST treat a cross-endorsement involving an expired certificate as invalid (per Section 14, State 6).
